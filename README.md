...
This repository is a curated collection of the vulnerable contracts, "big brain" logic fails, and interesting exploits I stumble upon during my deep dives.
...
If a contract address is published here, it is because the contract is already "wrecked", it holds no assets or the attack would require special conditions impossible to meet in reality.
In cases where I discover an active, high-risk vulnerability (still does not happen LoL), I follow responsible disclosure. To prevent actual exploits, I anonymize all identifying data, including addresses and contract names. I also avoid posting full source code and will rename functions to obscure the original target.
